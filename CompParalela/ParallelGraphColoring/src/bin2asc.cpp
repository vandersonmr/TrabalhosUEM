...
char masks[ 8 ] = { 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, static_cast<char>(0x80) };
...
/* my changes */
void write_graph_DIMACS_ascii(char* file, function<void(int,int)> callback)
{
  ...
  {
    for ( j=0; j<=i; j++ )
      if ( get_edge(i,j) ) callback(i+1, j+1);
  }
}
